...
an encrypted database file (which must have been encrypted by this
program). After loading the desired file, select the file submenu
to access more features, including search, export and save
as an encrypted database file. To exit the program, simply
...
functionality (the integration of which could not be completed
before submission).
...
There are multiple layers of debug functionality built into this program.
They can be activated by entering debug.h and uncommenting the 
"#define DEBUG", "#define DETAILEDDEBUG", "#define GRAPHICALDEBUG" or
"#define EVILDEBUG" lines. This simply enables a few lines of code which
print information when running certain functions. More detail on
each of the debug modes is available in debug.h.

******************************************************************

README written by Michael Lardner,
edited by Owen Dowley
2019